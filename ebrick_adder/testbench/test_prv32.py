...
import ebrick_adder.ebrick as ebrick
from ebrick_adder.testbench.program.riscv import build_riscv_binary
from ebrick_adder.testbench.umi_ram import UmiRam
...
def run_test(trace=False, fast=False):
    ...
    #This line tells the Verilog compiler (Verilator) where to look for files referenced 
    # by an include directive within the source code. For example `include "some_header.vh"
    dut.add('option', 'idir', 'testbench', package='ebrick_demo')
    #This line explicitly tells SiliconCompiler which file is the source code that needs to be compiled.
    ...
    # Processor with Python Monitor Connection
    mon = UmiTxRx('mtr2core_0.q', 'core2mtr_0.q', fresh=True)
    # Python Monitor with GPIO connection
    gpioq = UmiTxRx('host2gpio_0.q', 'gpio2host_0.q', fresh=True)
    # Python Monitor with Adder Connection
    adderq = UmiTxRx('host2adder_0.q', 'adder2host_0.q', fresh=True)
    ...
    # as a byte array.
    # The 'hello.bin' is a file created by the program that contains 
    # the compiled RISC-V program binary.
    ...
    # This object conceptually represents the entire physical memory space available to the processor
    main_memory = UmiRam(MEMORY_SIZE)
    # This method loads the byte array (program_mem) containing the RISC-V instructions and data into 
    # the UmiRam object, starting at memory address 0.
    ...
    # The while True loop runs indefinitely, constantly checking the queues
    while True:
        ...
        # If p is not None, a UMI Request packet has arrived from the PicoRV32.
        if p is not None:
            # The script immediately checks the command field (p.cmd) of the packet 
            # to verify it is one of the expected request types (Read, Write, or Posted Write).
            ...
            #--------------------------------------------------------------------------------------------------
            # This block is the dedicated logic within the main simulation loop that services all memory access 
            # requests (instruction fetches and data loads/stores) originating from the PicoRV32 processor.
            #--------------------------------------------------------------------------------------------------
            # 0x0000 is the chipid for UmiRam
            # 0x00_0000_0000 (40bits) to MEMORY_SIZE is address space for main memory
            if p.dstaddr < ((0x0000 << 40) + MEMORY_SIZE):
                if opcode in {UmiCmd.UMI_REQ_WRITE, UmiCmd.UMI_REQ_POSTED}:
                    ...
                    # change the command to a read response. Erases the code for UMI_REQ_READ
                    # to the required response opcode UMI_RESP_READ. This is achived by using 
                    # a mask to erase the last 5 bits - where the code is located
            ...
            elif ((p.dstaddr >> 40) & 0xFFFF) == 0xAAAA:
                adderq.send(p)

            #--------------------------------------------------------------------------------------------------
            # This block implements the address decoding for the I/O and Control peripherals that are handled 
            # directly by the Python monitor (Chip ID 0xCCCC). It defines where the UART output and the simulation 
            # exit signals reside in the chip's memory map.
            #--------------------------------------------------------------------------------------------------
            # 0xCCCC is the chipid for the Python monitor
            # 0x00_C000_0000 is the write address for the UART device
            # The CCCC with forty shifts is CC CC00 0000 0000. If we add the starting address, we have 
            # CC CC00 C000 0000. If the incoming UMI packet targets this specific address, it means the PicoRV32 
            # program executed an instruction to print a character.
            ...
            #--------------------------------------------------------------------------------------------------
            # This final block in the monitoring loop ensures that the UMI protocol's handshaking requirements 
            # are met for all standard write transactions processed by the monitor.
            #--------------------------------------------------------------------------------------------------
        ...
        # Check for responses FROM the Adder
        a = adderq.recv(blocking=False)
        
        if a is not None:
            # Determine if the packet from the adder is a read or write response
            opcode = umi_opcode(a.cmd)
            
            # If it's a read response, it contains the result. Print it.
            if opcode == UmiCmd.UMI_RESP_READ:
                result = int(a.data.view(np.uint32)[0])
                print(f"*** RESULT FROM ADDER: {result} ***")
